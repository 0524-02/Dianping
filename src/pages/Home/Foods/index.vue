<template>
<!-- foods最外层 -->
  <div class="foods-container">
    <div class="container">
      <!-- 左右foods外层 -->
      <div class="foods-wrap turnleft" v-for="(item,index) in foodDatas" :key="index">
        <!-- foods头部 -->
        <div class="foods-title" >
          <div class="cate-title">
            <div class="iconfont iconicon-test cateleft"></div>
            <div class="cateleft cates"><span>美食</span></div>
            <div class="cateleft cate"> <a href="">小吃快餐</a> </div>
            <div class="cateleft cate"><a href="">粉面馆</a></div>
            <div class="cateleft cate"><a href="">水果生鲜</a></div>
            <div class="cateright next"><a href=""> > </a></div>
            <div class="cateright all">
              <!-- <a href="">全部</a> -->
              <router-link to="/hotels">全部</router-link>
            </div>
    
          </div>
        </div>
        <!-- foods详情容器 -->
        <div class="foods-detail"  >
          <!-- foods上下层 -->
          <div class="detail-item" >
            <!-- 单个food -->
            <div class="itemcontainer" v-for="(item,index) in foodDatas.foodLists1" :key="item.categoryId">
                <div class="Img">
                  <img :src="item.url" alt="">
                </div>
                <div class="context-wrap">
                    <div class="name">
                      <a href="" class="name-desc">{{item.shopName}}</a>
                      <i class="iconfont icontuangou1"></i>
                    </div>

                    <div class="appraise">
                      <div class="star">
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                      </div>
                      <div class="comment"><span>{{item.evaluate}}</span></div>
                    </div>
                    
                    <div class="area-info">
                      <div class="maincate-name"><span>{{item.snack}}</span></div>
                      <div class="maincate-name location"><span>{{item.location}}</span></div>
                    </div>

                    <div class="avg">
                      <span>￥</span>
                      <span>{{item.price}}</span>
                      <span>/人</span>                   
                    </div>
                </div>
            </div>  
            <!-- <div class="itemcontainer">
                <div class="Img">
                  <img src="./images/foods1.jpg" alt="">
                </div>

                  <div class="context-wrap">

                    <div class="name">
                      <a href="" class="name-desc">福满楼</a>
                      <i class="iconfont icontuangou1"></i>
                    </div>

                    <div class="appraise">
                      <div class="star">
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                      </div>
                      <div class="comment"><span>22129条评价</span></div>
                    </div>
                    
                    <div class="area-info">
                      <div class="maincate-name"><span>老北京火锅</span></div>
                      <div class="maincate-name"><span>朝外大街/世贸天阶</span></div>
                    </div>

                    <div class="avg"><span>135￥/人</span></div>
              </div>
            </div>
            <div class="itemcontainer">
                <div class="Img">
                  <img src="./images/foods1.jpg" alt="">
                </div>

                  <div class="context-wrap">

                    <div class="name">
                      <a href="" class="name-desc">福满楼</a>
                      <i class="iconfont icontuangou1"></i>
                    </div>

                    <div class="appraise">
                      <div class="star">
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                      </div>
                      <div class="comment"><span>22129条评价</span></div>
                    </div>
                    
                    <div class="area-info">
                      <div class="maincate-name"><span>老北京火锅</span></div>
                      <div class="maincate-name"><span>朝外大街/世贸天阶</span></div>
                    </div>

                    <div class="avg"><span>135￥/人</span></div>
              </div>
            </div>   -->
          </div>

          <div class="detail-item">
            <!-- 单个food -->
            <div class="itemcontainer" v-for="(item,index) in foodDatas.foodLists2" :key="item.categoryId">
                <div class="Img">
                  <img :src="item.url" alt="">
                </div>

                  <div class="context-wrap">

                    <div class="name">
                      <a href="" class="name-desc">{{item.shopName}}</a>
                      <i class="iconfont icontuangou1"></i>
                    </div>

                    <div class="appraise">
                      <div class="star">
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                      </div>
                      <div class="comment"><span>{{item.evaluate}}</span></div>
                    </div>
                    
                    <div class="area-info">
                      <div class="maincate-name"><span>{{item.snack}}</span></div>
                      <div class="maincate-name location"><span>{{item.location}}</span></div>
                    </div>

                    <div class="avg">
                      <span>￥</span>
                      <span>{{item.price}}</span>
                      <span>/人</span> 
                    </div>
              </div>
            </div>

            <!-- <div class="itemcontainer">
                <div class="Img">
                  <img src="./images/foods1.jpg" alt="">
                </div>

                  <div class="context-wrap">

                    <div class="name">
                      <a href="" class="name-desc">福满楼</a>
                      <i class="iconfont icontuangou1"></i>
                    </div>

                    <div class="appraise">
                      <div class="star">
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                      </div>
                      <div class="comment"><span>22129条评价</span></div>
                    </div>
                    
                    <div class="area-info">
                      <div class="maincate-name"><span>老北京火锅</span></div>
                      <div class="maincate-name"><span>朝外大街/世贸天阶</span></div>
                    </div>

                    <div class="avg"><span>135￥/人</span></div>
              </div>
            </div>

            <div class="itemcontainer">
                <div class="Img">
                  <img src="./images/foods1.jpg" alt="">
                </div>

                  <div class="context-wrap">

                    <div class="name">
                      <a href="" class="name-desc">福满楼</a>
                      <i class="iconfont icontuangou1"></i>
                    </div>

                    <div class="appraise">
                      <div class="star">
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                      </div>
                      <div class="comment"><span>22129条评价</span></div>
                    </div>
                    
                    <div class="area-info">
                      <div class="maincate-name"><span>老北京火锅</span></div>
                      <div class="maincate-name"><span>朝外大街/世贸天阶</span></div>
                    </div>

                    <div class="avg"><span>135￥/人</span></div>
              </div>
            </div>   -->
          </div> 
            
        </div>
      </div>

      <!-- 左右foods外层 -->
      <!-- <div class="foods-wrap turnleft"> -->
        <!-- foods头部 -->
        <!-- <div class="foods-title">
          <div class="cate-title">
            <div class="iconfont iconicon-test cateleft"></div>
            <div class="cateleft cates"><span>美食</span></div>
            <div class="cateleft cate"><a href="">小吃快餐</a></div>
            <div class="cateleft cate"><a href="">粉面馆</a></div>
            <div class="cateleft cate"><a href="">水果生鲜</a></div>
            <div class="cateright next"><a href=""> > </a></div>
            <div class="cateright all"><a href="">全部</a></div>
            
          </div>
        </div> -->
        <!-- foods详情容器 -->
        <!-- <div class="foods-detail"> -->
          <!-- foods上下层 -->
          <!-- <div class="detail-item"> -->
            <!-- 单个food -->
            <!-- <div class="itemcontainer">
                <div class="Img">
                  <img src="./images/foods1.jpg" alt="">
                </div>

                  <div class="context-wrap">

                    <div class="name">
                      <a href="" class="name-desc">福满楼</a>
                      <i class="iconfont icontuangou1"></i>
                    </div>

                    <div class="appraise">
                      <div class="star">
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                      </div>
                      <div class="comment"><span>22129条评价</span></div>
                    </div>
                    
                    <div class="area-info">
                      <div class="maincate-name"><span>老北京火锅</span></div>
                      <div class="maincate-name"><span>朝外大街/世贸天阶</span></div>
                    </div>

                    <div class="avg"><span>135￥/人</span></div>
              </div>
            </div>

            <div class="itemcontainer">
                <div class="Img">
                  <img src="./images/foods1.jpg" alt="">
                </div>

                  <div class="context-wrap">

                    <div class="name">
                      <a href="" class="name-desc">福满楼</a>
                      <i class="iconfont icontuangou1"></i>
                    </div>

                    <div class="appraise">
                      <div class="star">
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                      </div>
                      <div class="comment"><span>22129条评价</span></div>
                    </div>
                    
                    <div class="area-info">
                      <div class="maincate-name"><span>老北京火锅</span></div>
                      <div class="maincate-name"><span>朝外大街/世贸天阶</span></div>
                    </div>

                    <div class="avg"><span>135￥/人</span></div>
              </div>
            </div>

            <div class="itemcontainer">
                <div class="Img">
                  <img src="./images/foods1.jpg" alt="">
                </div>

                  <div class="context-wrap">

                    <div class="name">
                      <a href="" class="name-desc">福满楼</a>
                      <i class="iconfont icontuangou1"></i>
                    </div>

                    <div class="appraise">
                      <div class="star">
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                      </div>
                      <div class="comment"><span>22129条评价</span></div>
                    </div>
                    
                    <div class="area-info">
                      <div class="maincate-name"><span>老北京火锅</span></div>
                      <div class="maincate-name"><span>朝外大街/世贸天阶</span></div>
                    </div>

                    <div class="avg"><span>135￥/人</span></div>
              </div>
            </div>   -->
          <!-- </div> -->

          <!-- <div class="detail-item"> -->
            <!-- 单个food -->
            <!-- <div class="itemcontainer">
                <div class="Img">
                  <img src="./images/foods1.jpg" alt="">
                </div>

                  <div class="context-wrap">

                    <div class="name">
                      <a href="" class="name-desc">福满楼</a>
                      <i class="iconfont icontuangou1"></i>
                    </div>

                    <div class="appraise">
                      <div class="star">
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                      </div>
                      <div class="comment"><span>22129条评价</span></div>
                    </div>
                    
                    <div class="area-info">
                      <div class="maincate-name"><span>老北京火锅</span></div>
                      <div class="maincate-name"><span>朝外大街/世贸天阶</span></div>
                    </div>

                    <div class="avg"><span>135￥/人</span></div>
              </div>
            </div>

            <div class="itemcontainer">
                <div class="Img">
                  <img src="./images/foods1.jpg" alt="">
                </div>

                  <div class="context-wrap">

                    <div class="name">
                      <a href="" class="name-desc">福满楼</a>
                      <i class="iconfont icontuangou1"></i>
                    </div>

                    <div class="appraise">
                      <div class="star">
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                      </div>
                      <div class="comment"><span>22129条评价</span></div>
                    </div>
                    
                    <div class="area-info">
                      <div class="maincate-name"><span>老北京火锅</span></div>
                      <div class="maincate-name"><span>朝外大街/世贸天阶</span></div>
                    </div>

                    <div class="avg"><span>135￥/人</span></div>
              </div>
            </div>

            <div class="itemcontainer">
                <div class="Img">
                  <img src="./images/foods1.jpg" alt="">
                </div>

                  <div class="context-wrap">

                    <div class="name">
                      <a href="" class="name-desc">福满楼</a>
                      <i class="iconfont icontuangou1"></i>
                    </div>

                    <div class="appraise">
                      <div class="star">
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                          <i class="iconfont iconstar"></i>
                      </div>
                      <div class="comment"><span>22129条评价</span></div>
                    </div>
                    
                    <div class="area-info">
                      <div class="maincate-name"><span>老北京火锅</span></div>
                      <div class="maincate-name"><span>朝外大街/世贸天阶</span></div>
                    </div>

                    <div class="avg">
                      <span>￥</span>
                      <span>{{item.price}}</span>
                      <span>/人</span>  
                    </div>
              </div>
            </div>   -->
          <!-- </div> -->
          <div class="foods-wrap"></div>
        </div>
      <!-- </div> -->
    </div>
  <!-- </div> -->
 
</template>

<script>
import { mapState } from 'vuex'
export default {
  name: 'Foods',
  mounted() {
    this.getFoodDatas();
  },
  methods: {
    async getFoodDatas () {
      const result = await this.$store.dispatch('getFoodDatas')
    }
  },
  computed: {
    ...mapState({
      foodDatas: (state) => state.foods.foodDatas
    })
  }
}
</script>

<style lang="less" scoped>
// foods最外层
  .foods-container {
    margin-top: 40px;
    .container {
      width: 1190px;
      height: 480px;
      margin: 0 auto;
      background: #fff;
      // 左右foods外层
      .foods-wrap {
        position: relative;
        width: 590px;
        height: 480px;
        float: left;
        // 头部标题
        .foods-title {
          width: 590px;
          height: 70px;
          .cate-title {
            width: 550px;
            height: 30px;
            margin: 20px 20px;
            // border: 1px black solid;
            .cateleft{
              float: left;
              margin-right: 10px;
            }
            .cateright{
              float: right;
            }
            .turnleft {
              float: left;
            }
            .turnright {
              float: right;
            }
            .iconfont{
              width: 30px;
              height: 30px;
              line-height: 30px;
              border-radius: 50%;
              background: orange;
              text-align: center;
              opacity: 0.7;
            }
            .cates {
              height: 30px;
              line-height: 30px;
              font-size: 20px;
            }
            .cate {
              border: 1px solid #e1e1e1;
              height: 22px;
              line-height: 22px;
              cursor: pointer;
              padding: 0 5px;
              margin: 3px 10px 0 0;
              a {
                font-size: 12px;
                color: #777;
              }
            }
            .all {
              width: 24px;
              height: 16px;
              line-height: 16px;
              margin: 7px 7px  7px 0;
              a {
                color: #333;
                font-size: 12px;
                cursor: pointer;
                line-height: 14px;
              }
            }
            .next {
              margin-top: 4px;
              a {
                font-size: 16px;
                font-weight: bold;
                color: #ccc;
              }
            }


          }
        }

        // 详情容器
        .foods-detail {
          position: absolute;
          width: 550px;
          height: 410px;
          margin: 0 20px;
          // border: 1px solid red;
          top: 70px;
          // <!-- foods上下层 -->
          .detail-item {
            display: flex;
            width: 550px;
            height: 186px;
            // background: pink;
            justify-content: space-between;
            // <!-- 单个food -->
            .itemcontainer {
              width: 170px;
              height: 186px;
              // 图片容器
              .Img {
                width: 170px;
                height: 96px;
                img {
                  width: 100%;
                  height: 100%;
                }
              }
              // 文本容器
              .context-wrap {
                width: 170px;
                height: 78px;
                margin-top: 12px;
                // 福满楼
                .name {
                  position: relative;
                  width: 170px;
                  float: left;
                  .name-desc {
                    max-width: 118px;
                    display: inline-block;
                    font-size: 16px;
                    line-height: 19px;
                    color: #333;
                    margin-right: 5px;
                    white-space: nowrap;
                    overflow: hidden;
                    text-overflow: ellipsis;
                  }
                  .iconfont {
                    position: absolute;
                    right: 0;
                    color: #ff6b37;
                  }
                }
                // 评价
                .appraise {
                  display: flex;
                  width: 170px;
                  margin-top:10px;
                  justify-content: space-between                                                                                    ;
                  .star {
                    color: #ff6b37;
                    .iconfont {
                      font-size: 14px;
                    }
                  }
                  .comment {
                    span {
                      color: #999;
                    }
                    
                  }
                }

                // 老北京火锅
                .area-info {
                  display: flex;
                  justify-content: space-between;
                  .maincate-name {
                    width: 75px;
                    white-space: nowrap;
                    overflow: hidden;
                    text-overflow: ellipsis;         
                    span {                     
                      color: #999;
                    }
                  }
                  .location {
                    position: relative;
                    text-align: right;
                    span {
                      position: absolute;
                      right: 0;
                    }
                  }
                }

                // 价格
                .avg {
                  margin-top: 5px;
                  span {
                    color: #999;
                  }
                }
              }
              
            }
          }
          
        }       
      }  
      
      
    }
  }
</style>
